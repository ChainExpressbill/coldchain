...
func (aq *AccountQuery) QueryOrders() *OrderQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, account.OrdersTable, account.OrdersColumn),
		)
...
func (aq *AccountQuery) WithOrders(opts ...func(*OrderQuery)) *AccountQuery {
	query := &OrderQuery{config: aq.config}
...
func (aq *AccountQuery) sqlAll(ctx context.Context) ([]*Account, error) {
	...
	if query := aq.withOrders; query != nil {
		...
		query.Where(predicate.Order(func(s *sql.Selector) {
			s.Where(sql.InValues(account.OrdersColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.account_orders
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "account_orders" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_orders" returned %v for node %v`, *fk, n.ID)
			}
