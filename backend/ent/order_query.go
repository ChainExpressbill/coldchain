...
func (oq *OrderQuery) QueryManager() *AccountQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, order.ManagerTable, order.ManagerColumn),
		)
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withManager: oq.withManager.Clone(),
...
func (oq *OrderQuery) WithManager(opts ...func(*AccountQuery)) *OrderQuery {
	query := &AccountQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
	oq.withManager = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	...
	if query := oq.withManager; query != nil {
		ids := make([]string, 0, len(nodes))
		nodeids := make(map[string][]*Order)
		for i := range nodes {
			if nodes[i].account_orders == nil {
				continue
			}
			fk := *nodes[i].account_orders
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_orders" returned %v`, n.ID)
			}
			for i := range nodes {
